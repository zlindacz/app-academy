...
def heart_tracks
  ...
  execute(<<-SQL)
    SELECT albums.title, COUNT(tracks.*) AS count
    FROM albums
    JOIN tracks
      ON albums.asin = tracks.album
...
def song_title_counts
  ...
  execute(<<-SQL)
    SELECT tracks.song, COUNT(albums.*) AS count
    FROM tracks
    JOIN albums
      ON albums.asin = tracks.album
    GROUP BY tracks.song
    HAVING COUNT(albums.*) > 2
  SQL
end
...
def best_value
  ...
  # of tracks.
  execute(<<-SQL)
    SELECT albums.title, albums.price AS price, COUNT(tracks.*) AS count
    FROM albums
    JOIN tracks
      ON albums.asin = tracks.album
    GROUP BY albums.asin
    HAVING (albums.price / COUNT(tracks.*)) < 0.50
  SQL
end
...
def top_track_counts
  ...
  execute(<<-SQL)
    SELECT albums.title, COUNT(tracks.*) as track_count
    FROM albums
    JOIN tracks
      ON albums.asin = tracks.album
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT styles.style, (SUM(albums_with_count.price)/SUM(albums_with_count.count))
    FROM styles
    JOIN (
      ...
      GROUP BY albums.asin) as albums_with_count
      ON albums_with_count.asin = styles.album
    GROUP BY styles.style
    ORDER BY (SUM(albums_with_count.price)/SUM(albums_with_count.count)) DESC
    LIMIT 5
  SQL
end
