...
SELECT the column(s) that you GROUP by, as well as aggregate functions of those
...
- explicit grouping is when the GROUP BY clause is used; implicit is when aggregate is used.
- if you need to rtrieve additional columns along with columns generated by aggregate functions, then need to also GROUP BY to specify over which group of rows the aggregate functions should be applied

How do you count distinct values for a column across all members of the group?
add the word DISTINCT inside the count, but before the column name


How is NULL handled by aggregate functions?
they ignore NULL values
COUNT(*) counts rows, so does count NULL values
COUTN(val) counts nuber of values, not NULLs

8.3
How do you group on a single column?
select on the column you want to group and include an aggregate function of the metric you're interested in, then group by that column

How do you group on multiple columns?
select on all the column you want to group by, and the aggregate of the metric you're interested in, then group by those columns of interest

How do you group via expressions?
SELECT expression(data)
FROM ...
GROUP BY expression(data)

What are "rollups" used for?
the WITH ROLLUP option has the database server do some aggregate work for you and include the extra row in the generated table
