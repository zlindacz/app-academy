...
def cats_in_hats
  ...
  (1..100).each do |i|
    cats.map!.with_index do |cat, idx|
      (idx+1) % i == 0 ? cat = !cat : cat
    end
  end
  result = []
  cats.each_with_index { |cat, idx| result << (idx+1) if cat }
  result
end
...
def count_adjacent_sums(array, n)
  result = []
  until array.size <= 1
    array[0] + array[1] == n ? result << array.shift(2) : array.shift
  end
  result.size
end
...
def inventory_array(older, newer)
  older.to_h.merge(newer.to_h).sort.to_a
end
...
def word_with_most_repeats(sentence)
  ...
  sentence.split.each do |word|
    repeat[word] = number_of_repeats(word)
  end
  repeat.max_by { |word, repeats| repeats }.first
end
