...
def longest_palindrome(string)
  string.length.downto(1).each do |size|
    string.length.times do |start|
      break if size+start > string.length
      return string[start, size] if palindrome?(string[start, size])
    end
  end
  ''
end
...
def dasherize_number(num)
  ...
  number.map!.with_index do |num, idx|
    idx == 0 || num.to_i.even? && number[idx-1].to_i.even? ? num : "-#{num}"
  end
  number.join
end
...
def nearby_az(string)
  index = /a/ =~ string
  index ? string[index, 4].include?('z') : false
end
...
def hashtagify(sentence, words)
  words.each { |word| sentence = sentence.gsub!(/(#{word})/i, '#\1') }
  sentence
end
...
def range_summary(array)
  array.chunk_while { |i, j| i+1 == j }.map { |arr| [arr[0], arr[-1]] }
end
...
def vigenere_cipher(string, key_sequence)
  ...
  key = []
  string.length.times { key << key_sequence }
  key.flatten!

  string.chars.map.with_index { |letter, idx| alphabet[(alphabet.index(letter) + key[idx])%26] }.join
end
...
#   key = []
#   key << key_sequence while key.length < string.length
#   key.flatten!
...
def caesar_guesser(encrypted_string, alphabet)
  key = encrypted_string.chars.max_by { |letter| encrypted_string.count(letter) }
  offset = 'e'.ord - key.ord

  encrypted_string.chars.map { |letter| alphabet[(alphabet.index(letter) + offset) % 26] }.join
end
...
def in_all_strings?(long_strings, short_string)
  long_strings.all? { |long_string| long_string.include?(short_string) }
end
