...
def consecutive(arr)
  (arr.min..arr.max).to_a.count - arr.size
end
...
def largest_sum_pair(array)
  ...
  array.delete_at(max1_index)

  max2 = array.max
  max2_index = array.index(max2) < max1_index ? array.index(max2) : array.index(max2) + 1

  [[max1_index, max2_index].min, [max1_index, max2_index].max]
end
...
def number_counting_seq(n)
  string = ''
  n.times { |i| string = generate(string, i) }
  string
end

def generate(string, i)
  return '1' if i == 0
  string.chars.chunk_while { |i, j| i == j }.map { |arr| [arr.count.to_s, arr[0]] }.join
end
...
def greatest_common_factor(first_number, second_number)
  (factors(first_number) & factors(second_number)).max
end
...
def panoramic_pairs(landmarks)
  ...
  pairs << [landmarks.last, landmarks.first]
  pairs
end
...
def two_degrees_away(facebook, name)
  friends = facebook[name]
  friends_of_friends = []
  friends.each do |friend|
    friends_of_friends << facebook[friend]
  end
  friends_of_friends.flatten!.uniq - [name] - friends
end
...
def assign_pods(students, pods)
  hash = {}
  pods.each { |pod| hash[pod] = students.shift(4) }
  hash
end
...
def serial_killer_summary(calendar_of_murderers)
  spree = {}
  calendar_of_murderers.each_with_index { |killer, idx| spree[killer] ? spree[killer] << idx : spree[killer] = [idx] }
  
  spree.reject! do |killer, dates|
    dates.chunk_while { |i, j| i+1 == j }.to_a.size == dates.size
  end

  spree.each do |killer, dates|
    consecutive = dates.chunk_while { |i, j| i+1 == j }.to_a.select { |dates| dates.size > 1 }
    consecutive.map! { |dates| [dates[0], dates[-1]] }
    spree[killer] = consecutive
  end
