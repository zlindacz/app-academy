def bubble_sort(arr)
  ...
  until sorted
    ...
    arr.each_with_index do |num, idx|
      if idx < arr.size-1 && num > arr[idx+1]
...
def word_unscrambler(str, words)
  words.select { |word| str.chars.sort.join == word.chars.sort.join }
end
...
def ordered_vowel_words(str)
  str.split.select { |word| ordered_vowels?(word) }.join(' ')
end
...
def nearest_larger(arr, idx)
  ...
  while i >= 0 || j < arr.size
    return i if arr[i] > arr[idx]
    return j if arr[j] > arr[idx]
    i -= 1
    j += 1
  end
  nil
...
def caesar_cipher(offset, string)
  string.split.map { |word| caesar(word, offset) }.join(' ')
end
