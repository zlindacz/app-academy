...
def longest_palindrome(string)
  ...
  while length >= 0
    ...
    while start + length <= string.length
      substring = string.slice(start, length)
...
def longest_palindrome(string)
  string.length.downto(2).each do |length|
    (0...string.length).each do |start|
      substring = string.slice(start, length)
      break if start + length > string.length
      return substring if palindrome?(substring)
    end
  end
  ""
...
def dasherize_number(num)
  num_in_str = num.to_s.chars
  num_in_str.map!.with_index do |digit, idx|
    if (digit.to_i.odd? || num_in_str[idx-1].to_i.odd?) && idx != 0
      "-#{digit}"
    else
      digit
    end
...
def nearby_az(string)
  string.include?('a') ? string.slice(string.index('a'), 4).include?('z') : false
end
...
# # # key sequence [1, 2, 3], the result would be "ccqbpdtkqqcmbodt":9
# # #
